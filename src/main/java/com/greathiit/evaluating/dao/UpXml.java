...
public class UpXml extends HttpServlet {
...
public void doPost(HttpServletRequest request, HttpServletResponse response)
	 ...
	 PrintWriter out = response.getWriter();
	  ...
	  response.setContentType("text/html;charset=UTF-8");
	    ...
	    out.println("<script>"
	    		...
	    		+ "{"
	    		+ "var flag=parseInt(document.getElementById(\"wc\").innerHTML);if(flag==4)"
  ...
  {// 处理文件尺寸过大异常
	  ...
	  {
		out.println("<div><p>文件超过指定大小" + MAX_SIZE + "字节<p/></div>");
  ...
  {
	  out.println("<div><p>请选择上传文件<p/></div>");
  ...
  while (xc) 
  {
   ...
   {
		  out.println("<div><p>请选择以下文件上传：<p/></div>");
	   for (allowFlag = 0; allowFlag < allowedExtCount; allowFlag++)
		   out.println("*." + allowedExt[allowFlag]+ "&nbsp;&nbsp;&nbsp;");
    
    	out.println("<p /><a href=\"\\manager_web\\excelToSql.jsp\" target=\"_top\">返回</a>");
    return;
   }
   ...
   String u_name = QS+"/WEB-INF/Xml/"+student+prefix + "." + t_ext;
	   ...
	   try 
	   {
	    ...
	    out.println(u_name);
	    out.println("文件上传成功，已保存为: " + prefix + "." + t_ext+ " &nbsp;&nbsp;文件大小: " + size + "字节<p />");
	    
	    out.println("<div id='wc'>"+4+"</div>");
	    out.println(""
	    		+ "<div id='url'>"+u_name+"</div>");
	    out.println("</body>");
	    out.println("</html>");
