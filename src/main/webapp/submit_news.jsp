...
<%@ page
	import="java.io.*,com.greathiit.evaluating.dao.*,com.greathiit.evaluating.function.ComputeScore,com.greathiit.evaluating.mode.Item_student"%>
<%
...
<%
		...
		if (flag.equals("1001") || type.equals("stu")) {
			...
			int fill_access = Integer
					.parseInt(temp.getFieldValue("Sfill_access", (String) session.getAttribute("nowusername")));
			if (fill_access == 1) {
				out.print("2");//学生已经提交 没有修改权限 
				return;
			}
		} else {
			...
			temp.setFiled_name("user_number");
			int fill_accessB = Integer
					.parseInt(temp.getFieldValue("role", (String) session.getAttribute("nowusername")));

			temp.setTable("student");
			temp.setFiled_name("user_number");
			int fill_accessS = Integer
					.parseInt(temp.getFieldValue("now_role", (String) session.getAttribute("student_number")));
